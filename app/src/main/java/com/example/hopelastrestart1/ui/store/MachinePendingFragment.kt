...
class MachinePendingFragment : Fragment(), CellClickListenerMatPending {
    ...
    /*   private fun fetchMachineryDispatchedToMe(fmdispatchedByMe: GetMacAndMatDispbyMe) {
           viewModel.fetchMachineryDispatchedToMe(fmdispatchedByMe)
    ...
    private fun fetchAssignedMachAndMates(fetchAssignedMachAndMates: FetchAssignedMachAndMates) {
        viewModel.getAssignedMachinery(fetchAssignedMachAndMates)
            .observe(viewLifecycleOwner, Observer {
                it.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
                            ...
                            resource.data?.let { projects ->
                                if (projects.body() != null) {
                                    if (projects.body()?.status_code.toString().equals("200")) {
                                        val matDisp = projects.body()?.assigned_machinery
